...
from senta_cnn_python.net import cnn_net
from senta_cnn_python.processor import load_vocab, preprocess, postprocess
...
class SentaCNN(hub.Module):
    ...
    def context(
        ...
        Returns:
             ...
             outputs(dict): the output variables of senta_cnn (the sentiment prediction results)
             main_program(Program): the main_program of lac with pretrained prameters
        """
        main_program = fluid.Program()
        startup_program = fluid.Program()
        with fluid.program_guard(main_program, startup_program):
            with fluid.unique_name.guard("@HUB_senta_cnn@"):
                data = fluid.layers.data(
                    name="words", shape=[1], dtype="int64", lod_level=1)
                label = fluid.layers.data(
                    name="label", shape=[1], dtype="int64")

                cost, acc, pred, fc = cnn_net(data, label, 1256606)
