...
class Interventer(object):
    ...
    def load_dict(self):
        ...
        for line in io.open(self.user_dict_path, mode="r", encoding="utf-8"):
            ...
            assert wordfreq > 0, "Word frequency must be postive integer!"
            self.ngram_dict[word + "/" + pos] = np.log(
                wordfreq / self.total_count)
    ...
    def get_new_phrase_list(self, match_info, bound, query):
        ...
        """
        new_phrase_list = []
        phrase_left = query.ori_query_str[bound.left_char_bound:bound.
                                          start_index]
        phrase_right = query.ori_query_str[bound.end_index +
                                           1:bound.right_char_bound + 1]
        if phrase_left != "":
        ...
        if phrase_right != "":
            phrase_right += "/" + query.lac_query_list[bound.right_bound].split(
                '/')[1]
    ...
    def run(self, query):
        ...
        for match_info in self.user_dict.iter(query.ori_query_str):
            ...
            new_lm_score, new_phrase_list = self.get_new_phrase_list(
                match_info, bound, query)
        ...
        for bound, phrase_list in all_result:
            final_phrase_list += query.lac_query_list[last_index + 1:bound.
                                                      left_bound] + phrase_list
...
def parse_result(lines, crf_decode, id2label_dict, interventer=None):
    ...
    for sent_index in range(batch_size):
        ...
        tags = [id2label_dict[str(id)] for id in crf_decode[begin:end]]
        sent_out = []
        tags_out = []
        parital_word = ""
